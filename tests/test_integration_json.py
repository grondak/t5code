"""Integration tests for JSON-loaded trade goods with the rest of the system."""

import pytest
from pathlib import Path
from t5code import T5Lot, T5World, T5ShipClass, load_and_parse_t5_map, load_and_parse_t5_ship_classes
...
def test_imbalance_goods_cross_reference():
    """Test that ImbalanceTradeGood references resolve to valid classifications."""
    ...
    for good in imbalances_table.goods:
        ...
        # Note: Some imbalances reference "Ag" which needs to resolve to "Ag-1" or "Ag-2"
        # The system should handle this, but if "Ag" doesn't exist, skip name resolution
        try:
            name = good.get_name()
...
def test_specific_goods_accessible_by_index():
    """Test that specific goods can be accessed by classification, type, and index."""
...
def test_all_type_tables_have_six_items():
    ...
    for classification_code, classification_table in T5RTGTable.classifications.items():
...
def test_trade_goods_work_in_multiple_lot_creations(game_state):
    """Test that trade goods work correctly across multiple lot instantiations."""
    lots = []
