...
if __name__ == '__main__':
    ...
    test_load = T5Load('Rhylanor', GameState)
    print(f'The test load ID is {test_load.load_id} with mass {test_load.mass} and serial {test_load.serial}')
    print(f'Selling on worlds:')
    for world in ['Porozlo', 'Risek', 'Loneseda', 'Valhalla']:
        print(f'\tWorld: {world} trade classifications: {GameState.world_data[world].trade_classifications()} tech level: {letter_to_tech_level(GameState.world_data[world].UWP()[8:])} // load value: {test_load.determine_sale_value_on(world, GameState)}')
    test_world = 'Efate'
    ...
    test_load = T5Load(test_world, GameState)
    print(f'The test load ID is {test_load.load_id} with mass {test_load.mass} and serial {test_load.serial}')
    print(f'\tSelling World: {world} trade classifications: {GameState.world_data[world].trade_classifications()} tech level: {letter_to_tech_level(GameState.world_data[world].UWP()[8:])} // load value: {test_load.determine_sale_value_on(world, GameState)}')
